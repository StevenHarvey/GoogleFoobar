    ...
    for i in range(len(mat)):
        ...
        for j in range(i, len(mat)):
            if mat[j][i].numerator != 0:
                ...
                break
        if index == -1:
            raise ValueError('Elimination failed')
...
def inverse(mat):
    ...
    for i in range(len(tmat)):
        ...
        mat_inv.append(gausselim(tmat, values))
    return mat_inv
...
def solution(mat):
    ...
    l = 1
    for item in row:
        l = latteral(l, item.denominator)
    res = list(map(lambda x: int(x.numerator*l/x.denominator), row)) #Hah, I see what you did there -- With the lambda
    res.append(l)
    return res